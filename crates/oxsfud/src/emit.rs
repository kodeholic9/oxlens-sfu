...
impl EventBus {
    ...
    pub fn room<T: Serialize>(&self, room_id: &str, op: Op, body: &T, exclude: &[String]) {
        ...
        self.send(Envelope { session_id: String::new(), user_id: String::new(), room_id: room_id.to_owned(), target: String::new(), exclude: exclude.to_vec(), wire, pc_mode: String::new(), participant_type: 0, hidden: false, metadata: String::new() });
    }
    ...
    pub fn user<T: Serialize>(&self, room_id: &str, user_id: &str, op: Op, body: &T) {
        let wire = encode_json(&Header::msg(op.code(), 0), &serde_json::to_value(body).unwrap_or_default());
        self.send(Envelope { session_id: String::new(), user_id: String::new(), room_id: room_id.to_owned(), target: user_id.to_owned(), exclude: Vec::new(), wire, pc_mode: String::new(), participant_type: 0, hidden: false, metadata: String::new() });
    }
    ...
    pub fn lifecycle(&self, room_id: &str, created: bool) {
        ...
        self.send(Envelope { session_id: String::new(), user_id: String::new(), room_id: room_id.to_owned(), target: String::new(), exclude: Vec::new(), wire, pc_mode: String::new(), participant_type: 0, hidden: false, metadata: String::new() });
    }
}
