...
impl SfuService for Sfu {
    ...
    async fn handle(&self, req: Request<Envelope>) -> Result<Response<Envelope>, Status> {
        ...
        for r in out.sims {
            let codec = crate::keyframe::Codec::from_name(&r.codec);
            let _ = self
                .udp
                .send(crate::transport::udp::Cmd::SetSimulcast {
                    ufrag: r.ufrag,
                    mid: r.mid,
                    vssrc: r.vssrc,
                    codec,
        ...
        // ★**말과 배관은 한 묶음**이다 — 발언권이 딸려 움직인 요청(`AFFILIATION`)이 여기 온다.
        self.push_dc(out.dc).await;
